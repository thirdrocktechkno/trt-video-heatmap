var HeatMap = (function () {
  ...
  HeatMap.prototype.viewPattern = function(){
   ...
   HeatMap.prototype.draw = function(){
      ...
      document.getElementById(inst.target).style.position = 'relative';
      for (var i = 0, j = 0; i < inst.videoData.length; i++) {
        if (inst.videoData[i] == inst.videoData[i+1]) {
          ...
          if (inst.div[0] != undefined && inst.div[0] != "") {
            var x = document.getElementById(inst.target).querySelectorAll(".trtForward"); 
            if(x.length == 0){
              left = 0;
            }
            else{
            ...
            para.className = "trtForward";
            para.style.cssText = "position: absolute;height: 100%;top: 0;background:" + background +";width:"+ (inst.div.length * 100)/inst.totalLength +"%;left:"+ left +"%";
            document.getElementById(inst.target).appendChild(para);
          }
